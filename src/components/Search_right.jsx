...
const SearchRight = ({ company, jobProfile, location, salaryRange, btnStatus, prerequisite ,id }) => {
    ...
    const handleApply = async () => {
        ...
        try {
            // Fetch job details from 'jobList' collection
            const jobDoc = await getDoc(doc(db, "jobList", id)); // Replace "id" with the unique identifier for this job
    
            if (jobDoc.exists()) {
                // Get job data
                const jobData = jobDoc.data();
    
                // Save job data to 'appliedJobs' collection
                const docRef = doc(db, "appliedJobs", id); // Replace "id" with the unique identifier for this job
